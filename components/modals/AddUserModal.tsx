...
export function AddUserModal({ open, onClose, onAdd }: Props) {
  ...
  async function submit() {
    ...
    if (!form.department.trim())
      nextErrors.department = "Department / Unit is required.";
    ...
    try {
      ...
      if (!userRes.ok || !userJson.data) {
        toast.error(
          userJson.error ?? "Failed to create user. Please try again."
        );
        return;
      }
      ...
      if (avatarFile) {
        ...
        if (avatarRes.ok && avatarJson.data?.fileUrl) {
          ...
          console.warn(
            "[AddUserModal] Avatar upload failed:",
            avatarJson.error
          );
          toast.error(
            "User created, but profile photo upload failed. You can set it later."
          );
        }
      }
      ...
      toast.success(
        `User "${fullName}" created. A temporary password has been sent to ${result.data.email}.`
      );
      onAdd?.(newUser);
      resetAndClose();
    } catch (err: any) {
    ...
    >
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Optional. Click the avatar to upload.
              <br />
        ...
        <div>
          ...
          <input
            className={cls("rank")}
            placeholder="e.g. P/Maj., P/Insp., P/Col."
            value={form.rank}
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Operations, Intelligence, Administration"
        ...
        <p className="text-xs text-slate-400 leading-relaxed">
          A temporary password will be sent to the user's email. They will be
        ...
        <div className="flex justify-end gap-2.5 pt-1">
          ...
          <Button
            variant="primary"
            onClick={submit}
            disabled={hasMissingRequired || submitting}
          >
