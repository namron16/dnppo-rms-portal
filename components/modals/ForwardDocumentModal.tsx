...
//
// FIX (admin-orders-audit.md §4.6): the active-role list was refetched via
//   `get_active_roles()` every single time the modal opened, even though
//   roles change infrequently (only on account creation/deactivation).
//   Added a small module-level cache with a 60s TTL, shared across every
//   mount of this modal in the session. On open we serve the cached list
//   instantly (no loading spinner, no round trip) if it's still fresh, and
//   only hit the network again once it's stale. A manual "createAccount"
//   flow can still call `invalidateActiveRolesCache()` to force a refresh.
//
...
// ── FIX 4.6: module-level cache shared by every instance of this modal ────
// Roles rarely change within a session, so we don't need a fresh network
// round trip on every single "Forward" click.
const ACTIVE_ROLES_CACHE_TTL_MS = 60_000; // 60s
let activeRolesCache: { data: ActiveRole[]; fetchedAt: number } | null = null;
let inFlightRolesRequest: Promise<ActiveRole[]> | null = null;

/** Call this after creating/deactivating an account to force a fresh fetch. */
export function invalidateActiveRolesCache() {
  activeRolesCache = null;
}

async function fetchActiveRoles(): Promise<ActiveRole[]> {
  const now = Date.now();

  if (
    activeRolesCache &&
    now - activeRolesCache.fetchedAt < ACTIVE_ROLES_CACHE_TTL_MS
  ) {
    return activeRolesCache.data;
  }

  // Dedupe concurrent callers (e.g. two modals opened back-to-back) into a
  // single in-flight request instead of firing multiple RPCs at once.
  if (inFlightRolesRequest) return inFlightRolesRequest;

  inFlightRolesRequest = (async () => {
    ...
    const roles = (data ?? []) as ActiveRole[];
    activeRolesCache = { data: roles, fetchedAt: Date.now() };
    return roles;
  })();

  try {
    return await inFlightRolesRequest;
  } finally {
    inFlightRolesRequest = null;
  }
}
...
export function ForwardDocumentModal({
  ...
  const [selectedRecipients, setSelectedRecipients] = useState<Set<AdminRole>>(
    new Set()
  );
  const [isForwarding, setIsForwarding] = useState(false);
  const { toast } = useToast();

  // ── Dynamic recipient list (FIX 4.6: cached, TTL-based) ────────────────
  const [allRoles, setAllRoles] = useState<ActiveRole[]>(
    () => activeRolesCache?.data ?? []
  );
  ...
  useEffect(() => {
    ...
    // Serve from cache instantly if fresh — no spinner, no round trip.
    if (
      activeRolesCache &&
      Date.now() - activeRolesCache.fetchedAt < ACTIVE_ROLES_CACHE_TTL_MS
    ) {
      setAllRoles(activeRolesCache.data);
      return;
    }
    ...
    fetchActiveRoles()
      ...
      .catch((err) => {
        if (cancelled) return;
        console.error(
          "[ForwardDocumentModal] Failed to load roles:",
          err.message
        );
  ...
  const handleForward = async () => {
    if (missingDriveMetadata) {
      toast.error(
        "This document is missing Drive metadata. Please re-upload it before forwarding."
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Forwarded to ${result.count} recipient${
            result.count !== 1 ? "s" : ""
          }.`
        );
        onForwarded();
      ...
      <div className="p-4 md:p-5 space-y-4">
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {document.type} · {attachments.length} attachment
          ...
          <div className="flex items-start gap-3 px-4 py-3 bg-amber-50 border border-amber-300 rounded-lg">
            ...
            <div className="text-xs text-amber-800 space-y-1">
              <p className="font-semibold text-amber-900">
                Cannot forward — missing Drive file
              </p>
              <p>
                This document was uploaded before the Google Drive system was
                set up, so it has no Drive file ID or storage account linked to
                it.
              </p>
              <p>
                To fix this,{" "}
                <strong>delete this document and re-upload it</strong> using the
                normal upload button. Once re-uploaded, it will have the Drive
        ...
        {!missingDriveMetadata && selectedRecipients.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800 space-y-1">
            ...
            <p>
              • To: <strong>{Array.from(selectedRecipients).join(", ")}</strong>
            </p>
            <p>
              • Attachments:{" "}
              <strong>
