...
function buildAdminUser(
  ...
  return {
    ...
    nav_group:
      (metadata?.nav_group as string | undefined) ?? inferNavGroup(role),
...
export function maskEmail(email: string): string {
  ...
  const masked =
    local.length > 1 ? "*".repeat(Math.min(local.length - 1, 4)) : "";
  return `${visible}${masked}@${domain}`;
}

// ── Safe sign-out helpers ─────────────────────────────────────────────────────

async function safeSignOutLocal(
  supabase: ReturnType<typeof createClient>
): Promise<void> {
...
async function safeSignOutGlobal(
  supabase: ReturnType<typeof createClient>
): Promise<void> {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const resolveEmailByRole = useCallback(
    async (role: string): Promise<string | null> => {
      const { data, error } = await supabase.rpc("get_email_by_role", {
        p_role: role,
      });
      if (error || !data) return null;
  ...
  const loginPassword = useCallback(
    async (role: string, password: string) => {
      ...
      try {
        ...
        await safeSignOutGlobal(supabase);
        return {
          error: "Could not establish a session lock. Please try again.",
        };
      }
  ...
  const logout = useCallback(async () => {
    ...
    if (roleForLog) {
      try {
        ...
        /* never block logout on log failure */
      }
    }
  ...
  const changePassword = useCallback(
    async (current: string, next: string) => {
      if (!user?.email)
        return { error: "Session error. Please log out and back in." };
    ...
    async (role: string) => {
      ...
      if (!email) {
        return {
          maskedEmail: null,
          error:
            "Could not send code. Please check your role selection and try again.",
        };
      }
  ...
  const verifyOTPAndReset = useCallback(
    async (role: string, token: string, newPassword: string) => {
      const email = await resolveEmailByRole(role);
      if (!email)
        return {
          error: "Could not verify identity. Please restart the reset flow.",
        };
