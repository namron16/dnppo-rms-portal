...
function LoginForm() {
  ...
  const handleOTPNext = useCallback(() => {
    ...
    if (trimmed.length !== 6 || !/^\d+$/.test(trimmed)) {
      setLoginError("The code must be exactly 6 digits.");
      return;
    }
  ...
  const handleLogin = useCallback(
    async (e: React.FormEvent) => {
      ...
      const supabase = createClient();
      const { data: emailData } = await supabase.rpc("get_email_by_role", {
        p_role: roleId,
      });
      ...
      if (otpError) {
        ...
        setView("login_otp_verify");
        setLoginError(
          "Incorrect or expired code. A new code has been sent — please check your email."
        );
  ...
  const handleFpSendOTP = useCallback(async () => {
    setFpError("");
    if (!fpRoleId) {
      setFpError("Please select your role first.");
      return;
    }
    setLoading(true);
  ...
  const handleResetPassword = useCallback(async () => {
    ...
    setLoading(true);
    const { error } = await verifyOTPAndReset(
      fpRoleId,
      fpOtpCode.trim(),
      newPw
    );
  ...
  return (
    <div className="w-full md:w-[460px] flex-shrink-0 bg-white flex flex-col md:h-screen overflow-y-auto shadow-2xl z-20">
      {/* ADD right after the opening tag — mobile-only compact header */}
      <div className="md:hidden flex items-center gap-3 px-6 pt-6 pb-2">
        ...
        />
        <div>
          <p className="text-[#1b365d] font-bold text-sm leading-tight">
            DNPPO Records
          </p>
        ...
        {view === "login_otp_request" && (
          <>
            ...
            <div className="space-y-4">
              ...
              {reason === "account_disabled" && (
                <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-xs leading-snug">
                  <span className="flex-shrink-0">🔒</span>
                  <span>
                    Your account has been disabled. Contact your system
                    administrator.
                  </span>
                </div>
              )}
              {reason === "session_taken" && (
                <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-red-50 border border-red-200 text-red-800 text-xs leading-snug">
                  ...
                  <span>
                    Your session was ended because this account was signed in
                    from another device.
                  </span>
                </div>
              )}
            ...
            <p className="text-center mt-3 text-[10px] text-slate-400 font-medium">
              Credentials are issued by your system administrator. No public
              registration.
            </p>
          </>
        )}
        ...
        {view === "login_otp_verify" && (
          <div>
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="font-serif text-xl text-[#1b365d] font-bold mb-1">
                Check Your Email
              </h2>
              ...
              {maskedEmail && (
                <p className="text-[#1b365d] font-semibold text-sm mt-0.5">
                  {maskedEmail}
                </p>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`${inputCls(
                    !!loginError
                  )} text-center text-xl tracking-[0.5em] font-bold font-mono`}
                  disabled={loading}
            ...
            <div className="mt-4 px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-[10px] text-amber-800 text-center">
                ⏱ The code expires in <strong>10 mins</strong>. Check spam/junk
                if not received.
              </p>
            </div>
          </div>
        ...
        {view === "login_password" && (
          <div>
            ...
            <div className="text-center mb-5">
              ...
              <p className="text-slate-500 text-xs">
                Signing in as{" "}
                <span className="font-semibold text-[#1b365d]">{roleId}</span>
              </p>
            </div>
        ...
        {view === "forgot_role" && (
          <div>
            ...
            <div className="text-center mb-5">
              ...
              <p className="text-slate-500 text-xs">
                Select your role and we'll send a 6-digit code to your
                registered email.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  value={fpOtpCode}
                  onChange={(e) => {
                    setFpOtpCode(e.target.value.replace(/\D/g, "").slice(0, 6));
                    setFpError("");
                  }}
                  placeholder="Enter 6-digit code"
                  className={`${inputCls(
              ...
              <button
                ...
                disabled={loading || fpOtpCode.length !== 6}
                className="w-full bg-[#1b365d] hover:bg-[#152a4a] text-[#fde047] font-semibold py-3 rounded-lg transition text-sm disabled:opacity-70 shadow-md flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
              ...
              <div>
                ...
                {newPw && (
                  <div className="mt-1.5 space-y-0.5">
                    ...
                    <p
                      className={`text-[10px] font-semibold ${PW_STRENGTH_TEXT[pwStrength]}`}
                    >
      ...
      <div className="px-10 py-4 border-t border-slate-100 flex items-center justify-center gap-3">
        <p className="text-[9px] text-slate-500 font-medium leading-tight text-center max-w-[220px]">
          Developed in collaboration with 4th-year BSIS students, Class 2026 of
          STI College Tagum.
        </p>
        <Image
...
export default function LoginPage() {
  return (
    <div className="min-h-screen md:h-screen overflow-y-auto md:overflow-hidden flex flex-col md:flex-row font-sans">
      {/* Left: Branding panel */}
      <div
        className="hidden md:flex flex-1 relative overflow-hidden flex-col justify-between px-12 py-8"
      ...
      >
        ...
        <div
          ...
          style={{ textShadow: "3px 2px 6px rgba(0,0,0,0.7)" }}
        >
          ...
          <div className="grid grid-cols-3 gap-2">
            {FEATURES.map((f) => (
              ...
              >
                ...
                <p className="text-[#fde047] text-[11px] font-bold leading-tight">
                  {f.title}
                </p>
