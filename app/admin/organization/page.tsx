...
function compareMembersByRank(a: OrgMember, b: OrgMember) {
  ...
  const posDiff = a.position.localeCompare(b.position, undefined, {
    sensitivity: "base",
  });
...
async function syncMembersToSupabase(members: OrgMember[]): Promise<boolean> {
  try {
    ...
    if (rows.length > 0) {
      ...
      if (upsertError) {
        console.warn(
          "Supabase unavailable (org_members upsert):",
          upsertError.message
        );
        return false;
      }
    ...
    if (existingError) {
      console.warn(
        "Supabase unavailable (org_members select ids):",
    ...
    if (deleteIds.length > 0) {
      ...
      if (deleteError) {
        console.warn(
          "Supabase unavailable (org_members delete stale):",
          deleteError.message
        );
        return false;
      }
...
function Lines({ layout }: { layout: Map<string, LayoutNode> }) {
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    for (const child of children) {
      lines.push(
        <line
          key={`vchild-${parentId}-${child.member.id}`}
          x1={child.x}
          y1={busY}
          x2={child.x}
          y2={child.y}
    ...
    >
      ...
      {member.unit && (
        <text
          ...
          fontFamily="Inter, sans-serif"
        >
          {member.unit.length > 30
            ? member.unit.slice(0, 29) + "…"
            : member.unit}
        </text>
      )}
      ...
      {member.contactNo && (
        <text
          ...
          fontFamily="Inter, sans-serif"
        >
          {member.contactNo.length > 24
            ? member.contactNo.slice(0, 23) + "…"
            : member.contactNo}
        </text>
      )}
      ...
      <div className="fixed z-[1200] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[430px] max-w-[93vw] bg-white rounded-2xl shadow-2xl overflow-hidden animate-fade-up">
        ...
        <div className="bg-red-600 px-6 py-5 flex items-start gap-3">
          ...
          <div>
            <p className="text-white font-bold text-[15px] leading-tight">
              Clear Entire Org Chart?
            </p>
            <p className="text-red-100 text-sm mt-1 leading-snug">
              All{" "}
              <strong className="text-white">members and connections</strong>{" "}
        ...
        <div className="px-6 py-5">
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5">
            <p className="text-red-800 text-sm font-bold mb-1">
              ⛔ This action cannot be undone.
            </p>
            <p className="text-red-700 text-xs leading-relaxed">
              All personnel cards, reporting lines, and hierarchy structure will
              be removed from local storage. You will need to rebuild the chart
              from scratch.
            </p>
          </div>
    ...
    >
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            {preview ? (
              ...
              <span className="text-xl font-bold" style={{ color: form.color }}>
        ...
        <div>
          ...
          <input
            className={cls}
            placeholder="e.g. Provincial Director"
            value={form.position}
            onChange={(e) =>
...
export default function OrganizationPage() {
  ...
  if (layout.size > 0) {
    ...
    canvasH =
      Math.max(...nodes.map((n) => n.y + CARD_H + ACTION_H + 20)) + CANVAS_PAD;
  }
  ...
  function handleZoom(delta: number) {
    setZoom((currentZoom) => {
      const nextZoom = Math.max(
        ZOOM_MIN,
        Math.min(ZOOM_MAX, +(currentZoom + delta).toFixed(2))
      );
  ...
  function handlePrintChart() {
    ...
    const scale = Math.min(
      1,
      pageWidthPx / chartWidth,
      (pageHeightPx - headerHeightPx) / chartHeight
    );
    ...
    layout.size === 0
      ...
      : Math.max(
          ...[...layout.values()].map((n) => {
            ...
            let cur = n.member;
            while (cur.parentId) {
              const p = displayMembers.find((m) => m.id === cur!.parentId);
              if (!p) break;
              cur = p;
              depth++;
            }
  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      <div className="flex flex-col flex-1">
        {/* Toolbar */}
        <div className="flex items-center gap-2.5 px-4 md:px-8 py-3 bg-white border-b border-slate-200 sticky top-14 z-40 flex-wrap">
          {isSuperAdmin && (
        ...
        <div className="p-2 md:p-6 flex-1">
            ...
            {members.length === 0 && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
                ...
                <p className="text-slate-400 text-sm">
                  Click <strong>+ Add Member</strong> to build your org chart.
                  <br />
                ...
                >
                  ...
                  {[...layout.values()].map((node) => (
                    <MemberCard
                      ...
                      onClick={() =>
                        setSelected((s) =>
                          s === node.member.id ? null : node.member.id
                        )
                      }
                      onEdit={() => {
                        setEditTarget(node.member);
                        setDefaultParentId(node.member.parentId);
                        setShowModal(true);
                      }}
          ...
          {members.length > 0 && (
            <div className="flex items-center gap-4 mt-3 text-xs text-slate-400 px-1">
              ...
              <span>
                {isLayoutEdit
                  ? "Layout Edit ON: drag canvas and zoom enabled"
                  : "Layout Edit OFF: canvas drag/zoom locked"}
              </span>
              <span>·</span>
              <span>
                Click card → use <strong>Edit</strong> ·{" "}
                <strong>Add Sub</strong> · <strong>Remove</strong> buttons below
                it
              </span>
            </div>
          )}
