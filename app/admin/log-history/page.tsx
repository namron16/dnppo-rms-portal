...
const ACTION_CONFIG: Record<
  string,
  { label: string; icon: string; badgeCls: string }
> = {
  // ── Session ────────────────────────────────────────────────────────────────
  login: {
    label: "Login",
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
      {[
        ...
        <div
          key={s.label}
          className={`${s.bg} border border-slate-200 rounded-xl px-4 py-3.5 flex items-center gap-3`}
        >
...
export default function LogHistoryPage() {
  ...
  const [roleMeta, setRoleMeta] =
    useState<Record<string, { color: string; name: string }>>(ROLE_META);
  ...
  const filtered = useMemo(() => {
    return logs.filter((log) => {
      ...
      const matchAction =
        actionFilter === "ALL" ||
        getActionConfig(log.action).label === actionFilter;
  ...
  // ── Filter options ───────────────────────────
  const allRoles = Object.keys(roleMeta).sort((a, b) =>
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        setRoleMeta((prev) => {
          ...
          data.forEach((r, i) => {
            merged[r.role] = {
              // ← remove the `if (!merged[r.role])` check
              name: r.display_name,
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${
                  realtime ? "bg-emerald-500 animate-pulse" : "bg-slate-300"
                }`}
              />
            ...
            <span>
              ...
              {filtered.length !== logs.length && (
                <>
                  {" "}
                  · filtered from{" "}
                  <strong className="text-slate-700">{logs.length}</strong>{" "}
                  total
                </>
              )}
            </span>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    {["Admin", "Action", "Description", "Date & Time"].map(
                      (h) => (
                        <th
                          key={h}
                  ...
                  {paginatedItems.map((log) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border ${cfg.badgeCls}`}
                          >
                            <span>{cfg.icon}</span>
                            {cfg.label}
                          </span>
