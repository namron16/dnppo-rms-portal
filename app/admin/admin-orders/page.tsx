...
import {
  ...
  type SOAttachmentUploadResult,
} from "@/components/modals/AddSpecialOrderAttachmentModal";
...
async function dbRenameAttachment(
  ...
  const { error } = await supabase
    ...
    .eq("id", id);
  if (error) {
    console.error("renameAttachment DB error:", error.message);
    return false;
  }
  return true;
}
...
async function dbDeleteAttachment(id: string): Promise<boolean> {
  const { error } = await supabase
    ...
    .eq("id", id);
  if (error) {
    console.error("deleteAttachment DB error:", error.message);
    return false;
  }
  return true;
}
...
async function uploadAttachmentToDrive(
  ...
  formData.append(
    "entityType",
    parentAttId ? "special_order_attachment" : "special_order"
  );
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2.5 pt-1">
          ...
          <Button
            ...
            disabled={
              !form.reference.trim() || !form.subject.trim() || !form.date
            }
          >
            💾 Save Changes
          </Button>
...
function AttachmentsTablePanel({
  ...
  const currentOrder =
    currentEntry.kind === "order" ? currentEntry.order : null;
  const currentLabel = isDrillDown
    ? displayName(
        (currentEntry as { kind: "attachment"; att: SOAttachment }).att
      )
    : `${currentOrder!.reference} – ${currentOrder!.subject}`;

  const rootOrderId = navStack[0].kind === "order" ? navStack[0].order.id : "";
  const parentAttId = isDrillDown
    ? (currentEntry as { kind: "attachment"; att: SOAttachment }).att.id
    : null;
  ...
  const drillAtt = isDrillDown
    ? (currentEntry as { kind: "attachment"; att: SOAttachment }).att
    : null;
  const drillFi = drillAtt
    ? fileInfoFromMime(drillAtt.mime_type, drillAtt.file_name)
    : null;
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[80px]">
                      Type
                    </th>
                    <th className="px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[90px]">
                      Size
                    </th>
                    <th className="px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[130px]">
                      Added
                    </th>
                    <th className="px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[90px]">
                      Children
                    </th>
                    <th className="px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[260px]">
                      Actions
                    </th>
                  </tr>
                </thead>
                              ...
                              <input
                                ...
                                onKeyDown={async (e) => {
                                  if (e.key === "Enter") {
                                    ...
                                    const ok = await onRenameAttachment(
                                      att,
                                      editingName
                                    );
                                    setRenamingId(null);
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                onDownloadFile(att.gdrive_url, label)
                              }
                            ...
                            {canEditOrder && (
                              <button
                                onClick={() => onForwardAttachment(att)}
                                className="inline-flex items-center justify-center px-1.5 py-1.5 text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 hover:border-blue-300 transition"
                            ...
                            <button
                              onClick={() =>
                                canEditOrder && onDeleteAttachment(att)
                              }
                              disabled={!canEditOrder}
                ...
                return (
                  <div key={att.id} className="p-3 sm:p-4 space-y-2">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-slate-500 flex-wrap">
                      ...
                      <span>·</span>
                      <span>
                        {new Date(att.created_at).toLocaleDateString("en-PH")}
                      </span>
                      {children > 0 && (
                        <button
                          onClick={() => onDrillDown(att)}
...
function OrderListNode({
  ...
  const topLevelCount = (attachmentsMap.get(order.id) ?? []).filter(
    (a) => !a.parent_id
  ).length;
  const statusColor =
    order.status === "ACTIVE"
...
export default function AdminOrdersPage() {
  ...
  const [forwardAttachment, setForwardAttachment] =
    useState<SOAttachment | null>(null);
  ...
  const [soAttachParentOrderId, setSoAttachParentOrderId] =
    useState<string>("");
  const [soAttachParentAttId, setSoAttachParentAttId] = useState<string | null>(
    null
  );
  ...
  const currentAttachments = useMemo((): SOAttachment[] => {
    ...
    if (currentEntry.kind === "order") {
      return (attachmentsMap.get(currentEntry.order.id) ?? []).filter(
        (a) => !a.parent_id
      );
    } else {
  ...
  async function handleAdd(newSO: SOWithUrl) {
    ...
    setOrders((prev) =>
      prev.some((o) => o.id === newSO.id) ? prev : [newSO, ...prev]
    );
    setAttachmentsMap((prev) => {
      const next = new Map(prev);
      next.set(newSO.id, []);
  ...
  async function handleArchiveOrder() {
    ...
    try {
      ...
      await logArchiveDocument(
        `${so.reference} – ${so.subject}`,
        "special order"
      );
  ...
  async function handleSaveOrder(updatedOrder: SOWithUrl) {
    ...
    await logAction(
      "edit_document",
      `Edited special order "${updatedOrder.reference} - ${updatedOrder.subject}"`
    );
    setOrders((prev) =>
  ...
  async function handleSOAttachmentModalResult(
    result: SOAttachmentUploadResult
  ) {
  ...
  async function handleDeleteAttachment() {
    ...
    try {
      ...
      deleteAttDisc.close();
      if (
        currentEntry?.kind === "attachment" &&
        currentEntry.att.id === att.id
      ) {
  ...
  return (
    <>
              ...
              {!currentEntry ? (
                ...
                <div className="space-y-4 min-w-0">
                  <AttachmentsTablePanel
                    ...
                    onArchiveOrder={() =>
                      selectedOrder && archiveDisc.open(selectedOrder)
                    }
                    onDeleteOrder={() =>
                      selectedOrder && deleteDisc.open(selectedOrder)
                    }
                    canEditOrder={canEditOrder}
                    onEditOrder={() =>
                      selectedOrder && editOrderDisc.open(selectedOrder)
                    }
      ...
      {/* Modals */}
      <AddSpecialOrderModal
        open={newSOModal.isOpen}
        onClose={newSOModal.close}
        onAdd={handleAdd}
      />
      ...
      {selectedOrder && (
        <ForwardDocumentModal
          ...
          document={{
            ...
            gdriveUrl:
              (selectedOrder as any).gdrive_url ?? selectedOrder.fileUrl ?? "",
