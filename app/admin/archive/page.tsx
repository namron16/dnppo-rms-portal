...
export default function ArchivePage() {
  ...
  const {
    ...
    setQuery,
    filtered: searched,
  } = useSearch(items, ["title", "archivedBy"] as Array<keyof ArchivedItem>);
  const filtered = searched.filter(
    (i) => typeFilter === "All Types" || i.type === typeFilter
  );
  ...
  async function handleRestore() {
    ...
    try {
      ...
      toast.error(
        `Failed to restore "${item.title}": ${err?.message ?? "Unknown error"}`
      );
    } finally {
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {paginatedItems.map((item) => (
                      ...
                      >
                        <td className="px-4 py-3.5 font-semibold text-sm text-slate-800">
                          {item.title}
                        </td>
