    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-9 z-50 w-44 rounded-xl border border-slate-200 bg-white shadow-lg py-1.5 px-1">
          ...
          {item.fileUrl &&
            menuItem("Download", <Download size={14} />, () =>
          ...
          {canDelete &&
            menuItem("Delete", <Trash2 size={14} />, onDelete, true)}
        </div>
      )}
    </div>
  );
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 space-y-2">
            <div>
              ...
              <Badge className={libraryBadgeClass(item.category)}>
                {item.category}
              </Badge>
            </div>
            <div>
        ...
        {item.fileUrl ? (
          <div className="border border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center bg-slate-50">
              ...
              <p className="text-sm text-slate-500 mb-3">
                Click "Open file" to view in a new tab.
              </p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-10 bg-slate-50 border border-slate-200 rounded-xl text-center">
            ...
            <p className="text-sm text-slate-400">
              No file attached to this library item.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((prev) => ({ ...prev, dateAdded: e.target.value }))
              }
            />
          </div>
        </div>
...
export default function LibraryPage() {
  ...
  const {
    ...
    setQuery,
    filtered: searched,
  } = useSearch(items, ["title"] as Array<keyof LibraryItemWithUrl>);
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([getLibraryItems(), getArchivedDocs()]).then(
      ([data, archived]) => {
  ...
  function handleAdd(newItem: LibraryItemWithUrl) {
    if (!canUploadLibrary) {
      toast.error("You do not have permission to add e-Library items.");
      return;
    }
  ...
  const handlePrintFile = useCallback(
    async (fileUrl: string, fileName: string, sourceDocumentId?: string) => {
      try {
    ...
    <>
              ...
              <EmptyState
                ...
                action={
                  !query && catFilter === "ALL" && canUploadLibrary ? (
                    <Button variant="primary" size="sm" onClick={newModal.open}>
                      + Add to Library
                    </Button>
                  ) : undefined
                }
                    ...
                    {paginatedItems.map((item) => (
                      ...
                      >
                        <td className="px-4 py-3.5">
                          ...
                          {item.description && (
                            <p className="text-xs text-slate-400 mt-0.5 ml-6 truncate max-w-xs">
                              {item.description}
                            </p>
                          )}
                        </td>
                ...
                {paginatedItems.map((item) => (
                  <div key={item.id} className="p-4 space-y-2">
                    ...
                    <p className="text-xs text-slate-500">
                      {item.size} · Added {item.dateAdded}
                    </p>
      ...
      {canForward && forwardPayload && user && (
        <ForwardDocumentModal
          ...
          onForwarded={() =>
            toast.success(`"${forwardPayload.title}" forwarded successfully.`)
          }
        />
      )}
