      ...
      <div className="p-6 space-y-4">
        <div className="grid gap-3 md:grid-cols-[1.5fr_1fr]">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 grid grid-cols-2 gap-3 text-xs">
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widests text-slate-400 mb-1">
                Author
              </p>
        ...
        {entry.fileUrl && (
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 flex items-center justify-between gap-3">
            ...
            <Button
              variant="outline"
              onClick={() => onViewAttachment(entry.fileUrl!, entry.title)}
            >
              View File
            </Button>
          </div>
...
function ViewJournalAttachmentModal({
  ...
  const isOffice = !!fileUrl.match(
    /\.(doc|docx|xls|xlsx|ppt|pptx|odt|ods|odp)(\?|$)/i
  );
...
export default function DailyJournalsPage() {
  ...
  const [activeType, setActiveType] = useState<"ALL" | JournalEntry["type"]>(
    "ALL"
  );
  ...
  const {
    ...
    setQuery,
    filtered: searched,
  } = useSearch(entries, ["title", "author", "content"] as Array<
    keyof JournalRecord
  >);
      ...
      try {
        ...
        setEntries(
          visible.map((entry: any) => ({
            ...
            summary:
              entry.summary ??
              (entry.content?.slice(0, 120) || "No summary available."),
            status: (entry.status ?? "Draft") as JournalStatus,
            attachments: entry.fileUrl
              ? Math.max(entry.attachments ?? 0, 1)
              : entry.attachments ?? 0,
          }))
        );
      } catch (error) {
  ...
  async function handleCreate(
    ...
    const nextEntry: JournalRecord = {
      ...
      content:
        input.content?.trim() || "No content was provided for this entry.",
  ...
  return (
    <>
          ...
          <div className="relative grid gap-6 p-6 lg:grid-cols-[1.4fr_1fr] lg:p-7">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Review daily memos, reports, and logs in a clean register. Use
                the filters to narrow the list and open any entry for a detailed
                view.
              </p>
        ...
        <div className="bg-white border-[1.5px] border-slate-200 rounded-xl overflow-visible flex flex-1 min-h-0 flex-col">
          <div className="flex items-center gap-2.5 px-3 sm:px-6 py-3 sm:py-4 border-b border-slate-100 bg-slate-50 flex-wrap">
            ...
            <ToolbarSelect
              value={activeType}
              onChange={(e) =>
                setActiveType(e.target.value as "ALL" | JournalEntry["type"])
              }
            >
          ...
          {loading ? (
            ...
            <div className="flex flex-1 min-h-0 items-center justify-center py-16">
              <EmptyState
                ...
                action={
                  !query && activeType === "ALL" && canUpload ? (
                  ...
                  <tbody>
                    {paginatedItems.map((entry) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5 align-top">
                          <Badge className={typeBadgeClass(entry.type)}>
                            {entry.type}
                          </Badge>
                        </td>
      ...
      <ViewJournalModal
        ...
        onViewAttachment={(fileUrl, _fileName) =>
