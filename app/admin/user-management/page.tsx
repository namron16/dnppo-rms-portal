...
import { listAllUsers, getSingleUser, setUserActive } from "./actions";
      ...
      {open && (
        ...
        >
          ...
          <button
            ...
            onClick={() => {
              close();
              onEditEmail(user.id, user.displayName, user.email);
            }}
          ...
          <button
            ...
            onClick={() => {
              close();
              onResetPW(user.id, user.displayName);
            }}
            className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[13px] text-slate-700
                       hover:bg-slate-50 transition text-left"
          >
          ...
          {user.isActive ? (
            <button
              ...
              onClick={() => {
                close();
                onToggleActive(user.id, user.isActive, user.displayName);
              }}
...
export default function UserManagementPage() {
  ...
  const [editEmailTarget, setEditEmailTarget] = useState<{
    id: string;
    displayName: string;
    email?: string;
  } | null>(null);
    ...
    try {
      ...
      setUsers(
        (
          raw as Omit<
            ManagedUser,
            "presenceActive" | "lastSeen" | "initials" | "avatarColor" | "title"
          >[]
        ).map((u) => {
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="ml-auto">
            <div
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-semibold border transition-all ${
                realtimeConnected
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border-[1.5px] border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="px-6 py-3 bg-slate-50 border-t border-slate-100">
              <p className="text-[11px] text-slate-400">
                🔒 Accounts are managed via Supabase Auth. Disabling an account
                immediately signs the user out everywhere. Passwords are reset
      ...
      {showCreateModal && (
        <CreateAccountModal
          ...
          onSuccess={(role) => {
            ...
            setMessage({
              type: "info",
              text: `Account "${role}" created successfully. Reloading…`,
            });
            void load();
          }}
        />
