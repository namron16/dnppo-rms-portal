...
export function CreateAccountModal({ onClose, onSuccess }: Props) {
  const [form, setForm] = useState({
    ...
    initials: "",
    avatar_color: "#2563eb",
    // FIX: nav_group now supports 'pd' — Provincial Director, view-only
    // access to all documents + the normal Forwarded Files page.
  ...
  async function handleSubmit() {
    ...
    if (!/^[A-Z0-9_]+$/.test(form.role.toUpperCase())) {
      setError(
        "Role ID must be letters, numbers, or underscores only (e.g. P11, FINANCE)."
      );
      return;
    }
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="officer@dnppo.gov.ph"
                className={inputCls}
              />
            </div>
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="Admin Officer — Finance"
                className={inputCls}
              />
            </div>
            ...
            <div>
              <label className={labelCls}>
                Initials{" "}
                <span className="text-slate-400 font-normal">(2 letters)</span>
              </label>
              <input
                type="text"
                maxLength={3}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                set(
                  "nav_group",
                  e.target.value as "documents" | "admin" | "dpda-dpdo" | "pd"
                )
              }
              className={inputCls}
            >
              ...
              <option value="admin">
                Admin-only (log history, user management, etc.)
              </option>
            </select>
            <p className="text-[10px] text-slate-400 mt-1">
              Controls which sidebar menu items the account sees.{" "}
              <strong>Documents</strong>: uploads + document nav.{" "}
              <strong>Deputy</strong>: reviews all documents.{" "}
              <strong>PD</strong>: views all documents (read-only) + normal
              Forwarded Files. <strong>Admin-only</strong>: system settings,
              backup & recovery, user management — no document access.
            </p>
          </div>
