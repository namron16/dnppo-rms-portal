...
export async function getSingleUser(userId: string) {
  ...
  return {
    ...
    initials:
      profile?.initials ?? (profile?.role ?? "UN").slice(0, 2).toUpperCase(),
...
export async function createAccount(input: {
  ...
  const { error: roleError } = await admin.rpc("register_role", {
    ...
    p_default_route:
      input.nav_group === "admin" ? "/admin/log-history" : "/admin/master",
  ...
  if (roleError)
    throw new Error(`Role registration failed: ${roleError.message}`);
...
export async function deleteAccount(userId: string, role: string) {
  ...
  if (PROTECTED_ROLES.includes(role)) {
    throw new Error(
      `The "${role}" account is protected and cannot be deleted.`
    );
  }
  ...
  const { error: registryError } = await admin.rpc("deactivate_role", {
    p_role: role,
  });
  if (registryError)
    throw new Error(`Registry deactivation failed: ${registryError.message}`);
...
// ── Fetch current role_registry row ──────────────────────────────────────────

export async function getRoleRegistryRow(role: string) {
  await assertSuperAdmin();
  const admin = getAdminClient();
  ...
  const { data, error } = await admin
    .from("role_registry")
    .select(
      "role, display_name, title, nav_group, can_upload, is_viewer_only, sort_order"
    )
    .eq("role", role)
    .single();

  if (error) throw new Error(`Could not load role settings: ${error.message}`);
  return data;
}

// ── Update an existing role's permissions ─────────────────────────────────────
// Uses the register_role RPC (ON CONFLICT DO UPDATE) so role_registry stays
// the single source of truth. Also patches user_metadata for any auth user
...
export async function updateRolePermissions(input: {
  ...
  const PROTECTED_ROLES = ["admin", "PD", "DPDA", "DPDO"];
  if (PROTECTED_ROLES.includes(input.role)) {
    throw new Error(
      `"${input.role}" is a protected role and cannot be edited here.`
    );
  }

  const admin = getAdminClient();

  const { error: registryError } = await admin.rpc("register_role", {
    p_role: input.role,
    p_display_name: input.display_name,
    p_title: input.title,
    p_nav_group: input.nav_group,
  ...
  if (registryError)
    throw new Error(`Role update failed: ${registryError.message}`);

  // Patch is_viewer_only + nav_group into user_metadata for any user on this role
  // (can_upload is re-read from role_registry at every login, no JWT patch needed).
  const { data: profiles } = await admin
    .from("profiles")
    .select("id")
    .eq("role", input.role);
  ...
  for (const p of profiles ?? []) {
    ...
    await admin.auth.admin.updateUserById(p.id, { user_metadata: merged });
  }
  ...
  await admin.from("admin_logs").insert({
    ...
    description: `Updated role permissions for "${input.role}": can_upload=${input.can_upload}, nav_group=${input.nav_group}, is_viewer_only=${input.is_viewer_only}`,
  });
