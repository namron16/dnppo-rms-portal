...
function classifyArchiveError(err: any): { message: string; status: number } {
  ...
  if (msg.includes('Folder resolution failed') || msg.includes('findOrCreateFolder'))
    return { message: 'Could not find or create the archive folder. Please try again.', status: 500 }

  return { message: 'Archive failed. Please try again or contact your admin.', status: 500 }
}
...
export async function POST(request: Request) {
  ...
  const { message, status } = classifyArchiveError(err)
